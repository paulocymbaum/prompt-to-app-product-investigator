...
async def async_client():
    ...
    prompt_routes._prompt_cache.clear()
...
"""
    ...
    storage = ConversationStorage(base_dir=str(tmp_path))
...
async def test_download_markdown_format(async_client, mock_storage):
    ...
    # Verify headers (content-type may or may not include charset)
...
@pytest.mark.asyncio
async def test_generate_prompt_session_not_found(async_client, tmp_path):
    """Test behavior when session file doesn't exist - generates minimal prompt."""
    ...
    # System generates a minimal prompt even for non-existent sessions
    # This is by design - allows preview of template
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
    assert "prompt" in data
...
@pytest.mark.asyncio
async def test_regenerate_prompt_session_not_found(async_client, tmp_path):
    """Test behavior when regenerating non-existent session - generates minimal prompt."""
    # Create empty storage
    storage = ConversationStorage(base_dir=str(tmp_path))
...
@pytest.mark.asyncio
async def test_download_prompt_session_not_found(async_client, tmp_path):
    """Test behavior when downloading from non-existent session - generates minimal prompt."""
    # Create empty storage
    storage = ConversationStorage(base_dir=str(tmp_path))
    ...
    # System generates and downloads a minimal prompt even for non-existent sessions
    assert response.status_code == status.HTTP_200_OK
    assert "attachment" in response.headers["content-disposition"]
...
async def test_generate_prompt_handles_validation_error(async_client, mock_storage):
    ...
    # Mock generator to raise ValueError - need to mock at the async method level
    async def mock_generate_error(session_id):
        raise ValueError("Incomplete investigation")
    
    with patch("routes.prompt_routes.get_conversation_storage", return_value=storage):
        with patch("routes.prompt_routes.PromptGenerator.generate_prompt", side_effect=mock_generate_error):
            response = await async_client.get(f"/api/prompt/generate/{session_id}")
...
async def test_multiple_sessions_isolated_cache(async_client, tmp_path):
    ...
    from routes.prompt_routes import get_conversation_storage
    ...
    # Create conversation files with correct format (using ----- delimiter)
    for session_id in [session1_id, session2_id]:
        conversation_file = conversation_dir / f"{session_id}.md"
        conversation_file.write_text(f"""**Question:** What is your product?
...
**Answer:** Users of {session_id}
**Timestamp:** 2024-01-01T12:01:00
""")
    
    # Create storage with tmp_path and override dependency
    storage = ConversationStorage(base_dir=str(tmp_path))
    app.dependency_overrides[get_conversation_storage] = lambda: storage
    ...
    try:
        ...
        assert "session-1" in prompt1
        assert "session-2" in prompt2
    finally:
