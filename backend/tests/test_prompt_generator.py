...
def test_suggest_architecture_realtime(prompt_generator):
    ...
    assert 'Event-Driven' in architecture or 'realtime' in architecture.lower()
...
def test_compile_additional_context(prompt_generator):
    ...
    answers = {
        'functionality': {
            'feature_one': 'Feature description here with sufficient length',
            'feature_two': 'Another detailed feature explanation',
            'feature_three': 'A third comprehensive feature description'
        },
        'market': {
            'competition': 'Detailed competitive analysis information',
    ...
    # Should include detailed entries or return None
    if context and context != 'None':
        assert 'Feature' in context or 'feature' in context.lower() or 'competitive' in context.lower()
    
    # Should filter short/common keys
    if 'overview' in context.lower():
        # If overview is included, it should not be the short one
